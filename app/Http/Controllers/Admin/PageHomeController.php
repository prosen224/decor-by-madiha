...
class PageHomeController extends Controller
{
    ...
    {
        $data['clients_title'] = $request->input('clients_title');
        $data['clients_subtitle'] = $request->input('clients_subtitle');
        $data['clients_status'] = $request->input('clients_status');

        PageHomeItem::where('id', 1)->update($data);
        return redirect()->back()->with('success', 'Clients Section is updated successfully!');
    }
